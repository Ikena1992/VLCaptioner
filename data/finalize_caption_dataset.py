...
def publish_files(sources, generated):
    """Stage a complete set, retaining originals until publication succeeds."""
    ...
    try:
        ...
        (stage / "transaction.json").write_text(
            json.dumps({"names": names, "backups": list(backups)}), encoding="utf-8"
        )
        for name in names:
            (stage / name).replace(DONE_DIR / name)
            installed.append(name)
        (stage / "committed.json").write_text(
            json.dumps([src.name for src in sources]), encoding="utf-8"
        )
    except Exception:
        ...
        for name in reversed(installed):
            if name in backups:
                ...
                (stage / "restore.tmp").replace(DONE_DIR / name)
            else:
                (DONE_DIR / name).unlink(missing_ok=True)
        shutil.rmtree(stage)
        raise
...
def finish_cleanup(stage):
    ...
    names = json.loads((stage / "committed.json").read_text(encoding="utf-8"))
    for name in names:
        if Path(name).name != name or not (DONE_DIR / name).is_file():
...
def process_caption_pair(short_path: Path, report_success: bool = True) -> bool:
    ...
    if not long_path.exists():
        print(f"SKIP {stem}: matching .long file not found")
        return False
    ...
    try:
        ...
        long_text = normalize_output_text(
            long_path.read_text(encoding="utf-8-sig")
        )
    ...
    if not short_text or not long_text:
        print(f"SKIP {stem}: caption file is empty")
        return False
    ...
    try:
        ...
        sources = [short_path, long_path, image_path, tag_path]
        if txt_path.exists():
            sources.append(txt_path)
        publish_files(sources, {
            short_path.name: short_text,
            long_path.name: long_text,
